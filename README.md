...
Using the [test dataset](https://github.com/Md-Khid/Linear-Regression-Modelling/blob/main/TestData.csv) allows for further evaluation of the model's performance on unseen data. Based on the model's performance, the predictions are relatively close to the actual values, indicating that the model generalises well to new data.
